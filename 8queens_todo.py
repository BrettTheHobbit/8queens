...
def fitness_8queen(individual):  # maximization
    ...
    for i in range(0, l):
        for j in range(i + 1, l):
            ...
            # Ensure that two queens overlap on a diagonal
            row1 = abs(individual[i])
            row2 = abs(individual[j])

            if (row1 - row2) == (i - j): #checks the forward and backward diagonals
                violations += 1 #if they do then add 1 to the violation score
...
def permutation_swap(individual):
    ...
    sol_length = len(individual)

    #pick two random incides that are not the same

    indices = random.sample(range(0, sol_length), 2) # Returns a selection of two possible indices from [0, length of solution]

    # swap their place in the mutant

    mutant[indices[0]],mutant[indices[1]] = mutant[indices[1]], mutant[indices[0]] 
...
def permutation_cut_and_crossfill(parent1, parent2):
    ...
    # student code begin
    # Choose random cutting point
    sol_length = len(parent1)
    cut_point = random.randint(1, sol_length-1) # gets our random cutting line as an int
    
    # copy the first part spliced before the crossover, paste into offspring
    offspring1 += parent1[0:cut_point]
    offspring2 += parent2[0:cut_point]

    second_half1 = parent1[cut_point:] # will be appended to offspring 2
    second_half2 = parent2[cut_point:] # Will be appended to offspring 1

    # Create second half, skip duplicates
    spares_list1 = []
    spares_list2 = []

    for num in second_half1: # if its not a duplicate existing in the first offspring, we take from solution 2 and append
        if num not in offspring2:
            offspring2.append(num)
        else:
            spares_list2.append(num)

    for num2 in second_half2: # if its not a duplicate existing in the second offspring, we take from solution 1 and append
        if num2 not in offspring1:
            offspring1.append(num2)
        else:
            spares_list1.append(num2)


    #take the skipped numbers and add them along with the rest of the second half
    offspring1.extend(spares_list2)
    offspring2.extend(spares_list1)
...
def tournament(fitness, mating_pool_size, tournament_size):
    ...
    available = list(range(len(fitness)))

    while len(selected_to_mate) < mating_pool_size:
        bracket = random.sample(available, tournament_size)

        winner = max(bracket, key=lambda idx: fitness[idx])  # index with best fitness
        selected_to_mate.append(winner)

        available.remove(winner)  # prevents selecting same winner again
...
if __name__ == "__main__":
    print(replacement([[1,2,3,4,5,6,7,8], [8,7,6,5,4,3,2,1], [1,2,4,5,6,7,8,3], [4,5,6,7,8,1,2,3]], [2, 3, 4, 5], [[1,2,3,4,8,7,6,5], [8,7,6,5,1,2,3,4]], [8,9])) #testing each function
    main()
